...
conversion_loop:
    ...
    # Store the quotient in %eax and the remainder in %edx (both implied).
    divl %edi

    # Quotient is in the right place.
    # %edx has the remainder, which now needs to be converted into a number.
    # So, %edx has a number that is 0 through 9.
    # You could also interpret this as an index on the ASCII table starting
    # from the character '0'. The ascii code for '0' plus zero is still
    # the ascii code for '0'. The ascii code for '0' plus 1 is the ascii
    # code for the character '1'. Therefore, the following instruction
    # will give us the character for the number stored in %edx
    addl $'0', %edx

    # Now we will take this value and push it on the stack.
    # This way, when we are done, we can just pop off the characters
    # one-by-one and they will be in the right order.
    # Note that we are pushing the whole register, but we only need
    # the byte in %dl (the last byte of the %edx register) for the character.
    pushl %edx
...
end_conversion_loop:
    # The string is now on the stack, if we pop it off a character
    # at a time we can copy it into the buffer and be done.
...
copy_reversing_loop:
    ...
    # So we are going to pop off to the entire %eax register, but then
    # only move the small part (%al) into the character string.
    popl %eax
    movb %al, (%edx)
